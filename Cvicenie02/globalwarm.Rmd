...
$\Delta X_t = X_t - X_{t-1} = \sum_{k=0}^pc_kt^k + Y_t - \sum_{k=0}^{p}c_k(t-1)^k - Y_{t-1}= \sum_{k=0}^{p}c_k(t^k-(t-1)^k) + \Delta Y_t$

Spravíme substitúciu:
$ (t-1)^k = t^k - kt^{k-1} + \binom{k}{2}t^{k-2} + ... + (-1)^k$

Dosadíme:
$ \sum_{k=0}^{p}c_k(t^k - kt^{k-1} + \binom{k}{2}t^{k-2} + ... + (-1)^k) + \Delta Y_t = c_pp t^{p-1}+ \sum_{k=0}^{p-2}b_kt^k + \Delta Y_t$
